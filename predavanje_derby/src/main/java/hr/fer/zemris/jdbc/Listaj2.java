...
public class Listaj2 {
	...
	public static void doOperation(DataSource dataSource) {
		...
		try {
			pst = con
					.prepareStatement("SELECT id, title, message, createdOn, userEMail from Poruke order by id");
			ResultSet rset = pst.executeQuery();
			try {
				while (rset.next()) {
					long id = rset.getLong(1); // ili rset.getLong("id");
					String title = rset.getString(2); // ili
														// rset.getString("title");
					String message = rset.getString(3); // ili
														// rset.getString("message");
					Date createdOn = rset.getTimestamp(4); // ili
															// rset.getTimestamp("createdOn");
					String userEMail = rset.getString(5); // ili
